...
from app.agent.reasoning import resolve_intent_policy
...
_FAST_PATH_RULES: tuple[tuple[frozenset[str], str], ...] = (
    (
        frozenset(
            {
                "你好",
    ...
    (
        frozenset({"谢谢", "多谢", "thanks", "thank you"}),
        "不客气。",
    ),
    (
        frozenset({"再见", "bye", "goodbye"}),
        "再见，有需要随时来问我。",
    ),
    (
        frozenset({"你是谁"}),
    ...
    (
        frozenset({"你能做什么", "你会什么"}),
        "我可以基于你保存的文章进行检索和回答，也可以在允许联网时补充网页资料。",
    ),
    (
        frozenset({"烦死了", "好烦", "无语", "靠"}),
        "有什么具体问题可以直接告诉我。",
    ),
    (
        frozenset({"滚", "妈的", "你真烦", "你真蠢"}),
        "如果你有具体问题，可以直接说。",
    ),
)
_FAST_PATH_TRAILING_PUNCTUATION = "！!。.？?，,"
_FAST_PATH_SPACED_ENGLISH_INPUTS = frozenset(
    {"hi", "hello", "thanks", "bye", "goodbye"}
)
_FAST_PATH_SPACED_ENGLISH_MAX_LENGTH = 20

# TODO(低优先级): 复杂混合表达和特殊格式仅在真实使用中高频出现时再增强。
...
def initialize(state: AgentState) -> dict[str, object]:
    ...
    return {
        ...
        "allow_web_override": state.get("allow_web_override"),
...
def non_knowledge_fast_path_check(state: AgentState) -> dict[str, object]:
    """仅为明确的非知识型短输入返回固定回答。"""

    answer = _match_fast_path_answer(state["original_query"])
    if answer is None:
        return {}

    return {
        **_answer_update(answer, [], None),
        "allow_web_override": None,
        "evidence_status": EvidenceStatus.SUFFICIENT,
        "evidence_reason": "非知识型短输入由固定规则直接响应。",
        "next_action": AgentAction.ANSWER,
    }


def route_after_non_knowledge_fast_path(
    state: AgentState,
) -> Literal["fast_path_answer", "main_agent"]:
    """命中固定回答时结束，否则进入现有 Agent 主链路。"""

    return "fast_path_answer" if state.get("final_answer") else "main_agent"


def _match_fast_path_answer(query: str) -> str | None:
    normalized = (
        query.strip()
        .lower()
        .rstrip(_FAST_PATH_TRAILING_PUNCTUATION)
        .strip()
    )
    answer = _find_exact_fast_path_answer(normalized)
    if answer is not None:
        return answer

    collapsed = _collapse_spaced_english_fast_path_input(normalized)
    if collapsed is None:
        return None
    return _find_exact_fast_path_answer(collapsed)


def _find_exact_fast_path_answer(normalized: str) -> str | None:
    for exact_inputs, answer in _FAST_PATH_RULES:
        if normalized in exact_inputs:
            return answer
    return None


def _collapse_spaced_english_fast_path_input(value: str) -> str | None:
    if len(value) > _FAST_PATH_SPACED_ENGLISH_MAX_LENGTH:
        return None

    tokens = value.split(" ")
    if len(tokens) < 2 or any(
        len(token) != 1 or not "a" <= token <= "z" for token in tokens
    ):
        return None
    ...
    return (
        collapsed
        if collapsed in _FAST_PATH_SPACED_ENGLISH_INPUTS
        else None
    )
...
def contextualize_query(
    ...
    conversation = _prior_conversation(state["messages"])
    if not conversation:
        return {"current_query": original_query}

    try:
        contextualized_query = runtime.context.reasoning.contextualize_query(
            original_query=original_query,
            conversation=conversation,
        )
    except Exception as exc:
        logger.warning(
            "Agent contextualize_query failed: %s",
            type(exc).__name__,
        )
        return {"current_query": original_query}

    return {"current_query": contextualized_query}
...
def evaluate_and_decide(
    ...
    kb_candidates = list(state.get("kb_results", []))
    web_candidates = list(state.get("web_results", []))
    article_fulltext_candidates = list(
        state.get("article_fulltext_evidence", [])
    )
    web_fulltext_candidates = list(state.get("web_fulltext_evidence", []))
    policy = runtime.context.policy
    ...
    if policy.should_force_web_for_freshness(state):
        return {
            "allowed_actions": allowed_actions,
            "selected_evidence": [],
            "evidence_status": EvidenceStatus.INSUFFICIENT,
            "evidence_reason": "时效性问题需要先尝试 Web Search。",
            "next_action": AgentAction.WEB_SEARCH,
            "selected_article_result_index": None,
            "selected_web_page_result_index": None,
    ...
    try:
        decision_options = (
            ...
            if _is_post_fulltext_decision(state)
            else {}
        )
        decision = runtime.context.reasoning.decide(
            original_query=state["original_query"],
            current_query=state["current_query"],
            kb_evidence=kb_candidates,
            web_evidence=web_candidates,
            article_fulltext_evidence=article_fulltext_candidates,
            web_fulltext_evidence=web_fulltext_candidates,
            allowed_actions=allowed_actions,
        ...
        if decision.next_action not in allowed_actions:
            ...
            logger.error(
                "Agent Decision selected unauthorized action: "
                "next_action=%s allowed_actions=%s allow_web=%s "
                "requires_freshness=%s intent=%s",
                ...
                intent.value if isinstance(intent, AgentIntent) else intent,
            )
            raise ValueError("模型选择了未授权动作")
        _validate_decision_evidence_indexes(
            decision,
            kb_candidate_count=len(kb_candidates),
            web_candidate_count=len(web_candidates),
            article_fulltext_candidate_count=len(article_fulltext_candidates),
            web_fulltext_candidate_count=len(web_fulltext_candidates),
        )
        ...
        logger.exception(
            "Agent evaluate_and_decide failed: %s",
            type(exc).__name__,
        )
...
def _validate_decision_evidence_indexes(
    decision: AgentDecision,
    *,
    kb_candidate_count: int,
    web_candidate_count: int,
    article_fulltext_candidate_count: int,
    web_fulltext_candidate_count: int,
) -> None:
    """按本轮候选快照校验四类回答证据索引。"""
    ...
    selections = (
        ...
        (
            ...
            decision.selected_article_content_indexes,
            article_fulltext_candidate_count,
        ),
        (
            "selected_web_page_content_indexes",
            decision.selected_web_page_content_indexes,
            web_fulltext_candidate_count,
        ),
    )
    for field_name, indexes, candidate_count in selections:
        if any(index < 0 or index >= candidate_count for index in indexes):
            valid_indexes = list(range(candidate_count))
            logger.error(
                "Agent Decision evidence index invalid "
                "field=%s indexes=%s candidate_count=%d valid_indexes=%s",
                field_name,
                indexes,
                candidate_count,
                valid_indexes,
...
def _is_post_fulltext_decision(state: AgentState) -> bool:
    """仅识别紧接知识库或网页全文工具之后的决策。"""

    last_result = state.get("last_tool_result")
    return isinstance(last_result, dict) and last_result.get("tool_name") in {
        "get_article_content",
        "fetch_web_page",
    }
...
def _prior_conversation(
    ...
    prior_messages = list(messages[:latest_user_index])
    has_prior_user = any(
        message.type == "human"
    ...
    return prior_messages[-6:] if has_prior_user else []
