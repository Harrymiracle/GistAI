...
def _serializer_from_checkpointer(monkeypatch) -> JsonPlusSerializer:
    ...
    with postgres_checkpointer("postgresql://user:password@localhost/db") as saver:
        return saver.serde
...
def test_checkpoint_serializer_round_trips_required_agent_types(
    monkeypatch,
) -> None:
    serializer = _serializer_from_checkpointer(monkeypatch)
    values = (
        AgentAction.ANSWER,
...
@pytest.mark.parametrize(
    "value",
    [AgentIntent.KNOWLEDGE_BASE_ONLY, EvidenceStatus.SUFFICIENT],
)
def test_checkpoint_serializer_does_not_allow_inline_only_agent_types(
    monkeypatch,
    value: StrEnum,
) -> None:
    serializer = _serializer_from_checkpointer(monkeypatch)
...
def test_checkpoint_serializer_blocks_unlisted_custom_type(monkeypatch) -> None:
    class UnlistedCheckpointType(StrEnum):
        VALUE = "unlisted"

    serializer = _serializer_from_checkpointer(monkeypatch)
    restored = serializer.loads_typed(
        serializer.dumps_typed(UnlistedCheckpointType.VALUE)
    )

    assert restored == "unlisted"
    assert not isinstance(restored, UnlistedCheckpointType)
...
def test_graph_and_checkpointer_recreation_recovers_conversation(
    ...
    with postgres_checkpointer(checkpoint_connection_string) as second_saver:
        ...
        restored_state = second_graph.get_state(
            {"configurable": {"thread_id": internal_thread_id}}
        ).values
        assert restored_state["allowed_actions"]
        assert all(
            type(action) is AgentAction
            for action in restored_state["allowed_actions"]
        )
        assert (
            restored_state["last_tool_result"]["status"]
